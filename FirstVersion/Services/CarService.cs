...
public class CarService(ICarRepository carRepository, IEmployeeRepository employeeRepository, IDriverRepository driverRepository) : ICarService
{
    ...
    public async Task<List<Car>?> GetAvailableCarsByCategoryAndCityAsync(Category category, string? city = null)
    {
        return await carRepository.GetAllCarsAsync(car => car.Category == category && car.City == city);
    }
